...
class ControllerPaymentMokaPayment extends Controller {
    ...
    public function index() {
        ...
        $data['action'] = $this->url->link('payment/moka_payment', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('payment/moka_payment.tpl', $data));
    ...
    public function update() {
      ...
      $updated = $this->model_payment_moka_payment->update($version_updatable);
        if ($updated == 1) {
			...
			$this->load->controller('payment/' . 'moka_payment' . '/uninstall');
			$this->load->controller('payment/' . 'moka_payment' . '/install');
            ...
            $this->response->redirect($this->url->link('payment/moka_payment', 'token=' . $this->session->data['token'] . "&update_error=$updated", true));
			}
    }
	...
	$moka_payment_test_mode = $this->config->get('moka_payment_test_mode');
	  ...
	  $url = 'https://service.moka.com/PaymentDealer/GetDealerPaymentTrxDetailList';
       ...
       return $this->load->view('payment/moka_payment_order.tpl', $data);
    }
